...
A를 B로 바꾸는 최소한의 명령어를 생성하는 프로그램이다. 예를 들어서 A = 1234, B = 3412 라면 다음과 같이 두 개의 명령어를 적용하면 A를 B로 변환할 수 있다.
...
n의 자릿수로 0 이 포함된 경우에 주의해야 한다. 예를 들어서 1000 에 L 을 적용하면 0001 이 
되므로 결과는 1 이 된다. 그러나 R 을 적용하면 0100 이 되므로 결과는 100 이 된다.
    ...
    while queue :
        ...
        for i in c :
            if i == 'D' :
                ...
                if len(str(x)) > 1 :
                    nx = int(str(x)[1:] + str(x)[0])
                else :
                    nx = x
            
            elif i == 'R' :
                if len(str(x)) > 1 :
                    nx = int(str(x)[-1] + str(x)[0:-1])
                else :
                    nx = x
